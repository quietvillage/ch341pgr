...
#define err( format, arg... )	\
    printk( KERN_ERR KBUILD_MODNAME "%s %d:" format "\n", __FILE__, __LINE__, ##arg)
	...
	size_t count, loff_t *file_pos)
{
	...
	dbg("count %ld,->bytes_read %ld,->mNewlen %ld,->mBuffer[0] %d,->[1] %d,->[2]%d,->[3]%d",count,bytes_read,mNewlen,mBuffer[0],mBuffer[1],mBuffer[2],mBuffer[3]);
	...
	size_t count, loff_t *file_pos)
{
	...
	buf = (char *)kmalloc( sizeof(unsigned char) * (mNewlen + mLength + 1), GFP_KERNEL );
	mutex_lock( &io_mutex );
				...
				unsigned long oBuffer, struct ch34x_pis *dev )
{
	...
	iLength -= 8;
	retval = copy_from_user(&mSave, (char __user*)(iBuffer + iLength), 1);
	retval |= copy_from_user(&readtimes, (char __user*)(iBuffer + iLength + 4), 1);
	if( retval != 0 )
	{
		err("copy error");
		//kfree( iBuf );
	...
	if( mSave * readtimes > MAX_BUFFER_LENGTH || mSave == 0 || readtimes == 0 )
	{
		return -EFAULT;
		goto exit;
	}
		...
		case CH34x_FUNCTION_SETPARA_MODE:
			{
				...
				dbg("------>SetParaMode - ch34x_arg %lx", ch34x_arg);
				break;
			}
